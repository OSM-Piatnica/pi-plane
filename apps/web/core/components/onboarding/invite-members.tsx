    ...
    <div>
      ...
      {email && !emailRegex.test(email) && (
        <div className="mx-8 my-1">
          ...
          <span className="mt-1 text-11 text-danger-primary">{t("onboarding.team.invalid_email_hint")}</span>
        </div>
      )}
    </div>
  );
    ...
    await workspaceService
      ...
      .then(async () => {
        setToast({
          ...
          message: t("onboarding.team.toast.invite_success_message"),
        });
        await nextStep();
      })
      .catch((err) => {
      ...
      <div className="h-full w-full overflow-auto px-6 py-10 sm:px-7 sm:py-14 md:px-14 lg:px-28">
        <div className="mx-auto mt-6 flex w-full flex-col items-center justify-center p-8 md:w-4/5">
          <div className="mx-auto w-4/5 space-y-1 py-4 text-center">
            <h3 className="text-24 font-bold text-primary">{t("onboarding.team.invite_teammates")}</h3>
            <p className="font-medium text-placeholder">{t("onboarding.team.invite_teammates_description")}</p>
          </div>
          <form
