  ...
  return invitations && invitations.length > 0 ? (
    <div className="space-y-4">
      <div className="mx-auto space-y-1 py-4 text-center">
        <h3 className="text-24 font-bold text-primary">{t("onboarding.workspace.invited_title")}</h3>
        <p className="font-medium text-placeholder">{t("onboarding.workspace.invited_description")}</p>
      </div>
      <div>
        {invitations &&
      ...
      >
        {isJoiningWorkspaces ? <Spinner height="20px" width="20px" /> : t("onboarding.workspace.continue_to_workspace")}
      </Button>
      ...
      >
        {t("onboarding.workspace.create_own")}
      </Button>
    </div>
  ) : (
    <div>{t("onboarding.workspace.no_invitations_found")}</div>
  );
}
